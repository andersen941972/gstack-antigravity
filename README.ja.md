...
### 3. ワークフローの反映
スクリプトが成功すると、同ディレクトリ内に `.agents/workflows/` というフォルダが生成されます。
これを、**あなたのプロジェクト（`project-example/`）のルート・ディレクトリ**へコピー、またはシンボリックリンクを作成してください。

```bash
# 例: ワークフローフォルダをプロジェクトのルートへコピーする
cd ../.. # project-example のルートへ戻る
cp -r tools/gstack_antigravity/.agents .
```
インストール後は再起動等は不要です。Antigravityはルートの `.agents/workflows/` フォルダを自動検知し、即座にコマンドが利用可能になります。
